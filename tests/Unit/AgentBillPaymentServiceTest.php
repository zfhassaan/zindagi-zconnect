...
class AgentBillPaymentServiceTest extends TestCase
{
    ...
    public function test_successful_agent_bill_payment(): void
    {
        ...
        $response = $service->agentBillPayment($this->requestDto());

        $this->assertInstanceOf(AgentBillPaymentResponseDTO::class, $response);
        $this->assertTrue($response->success);
        $this->assertEquals('99', $response->id);
    }

    public function test_agent_bill_payment_with_insufficient_balance_error(): void
    {
        $service = $this->makeService(
            logging: ['logInfo', 'logRequest', 'logResponse'],
            audit: true,
        ...
        $this->assertStringContainsString('Insufficient account balance', $response->message);
    }

    public function test_agent_bill_payment_with_invalid_json(): void
    {
    ...
    public function test_agent_bill_payment_with_network_error(): void
    {
        $mockClient = Mockery::mock(Client::class);
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('Failed agent bill payment', $response->message);
    }

    private function requestDto(): AgentBillPaymentRequestDTO
    {
    ...
    private function makeService(array $logging, bool $audit, Client $client): OnboardingService
    {
        ...
        $property = (new \ReflectionClass($service))->getProperty('agentBillPaymentClient');
