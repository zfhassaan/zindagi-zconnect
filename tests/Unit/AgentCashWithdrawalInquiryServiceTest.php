...
class AgentCashWithdrawalInquiryServiceTest extends TestCase
{
    ...
    public function test_successful_agent_cash_withdrawal_inquiry(): void
    {
        ...
        $response = $service->agentCashWithdrawalInquiry($this->requestDto());

        $this->assertInstanceOf(AgentCashWithdrawalInquiryResponseDTO::class, $response);
        $this->assertTrue($response->success);
        $this->assertEquals('88', $response->id);
    }

    public function test_agent_cash_withdrawal_inquiry_with_portal_error(): void
    {
        $service = $this->makeService(
            logging: ['logInfo', 'logRequest', 'logResponse'],
            audit: true,
        ...
        $this->assertEquals('8076', $response->errorCode);
        $this->assertStringContainsString('register the given Mobile Number', $response->message);
    }

    public function test_agent_cash_withdrawal_inquiry_with_invalid_json(): void
    {
    ...
    public function test_agent_cash_withdrawal_inquiry_with_network_error(): void
    {
        $mockClient = Mockery::mock(Client::class);
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('Failed agent cash withdrawal inquiry', $response->message);
    }

    private function requestDto(): AgentCashWithdrawalInquiryRequestDTO
    {
    ...
    private function makeService(array $logging, bool $audit, Client $client): OnboardingService
    {
        ...
        $property = (new \ReflectionClass($service))->getProperty('agentCashWithdrawalInquiryClient');
