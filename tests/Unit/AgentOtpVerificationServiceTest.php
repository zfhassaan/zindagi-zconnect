...
class AgentOtpVerificationServiceTest extends TestCase
{
    ...
    public function test_successful_agent_otp_verification(): void
    {
        ...
        $response = $service->agentOtpVerification($this->requestDto());

        $this->assertInstanceOf(AgentOtpVerificationResponseDTO::class, $response);
        $this->assertTrue($response->success);
        $this->assertEquals('99', $response->id);
    }

    public function test_agent_otp_verification_with_portal_error(): void
    {
        $service = $this->makeService(
            logging: ['logInfo', 'logRequest', 'logResponse'],
            audit: true,
        ...
        $this->assertEquals('9006', $response->errorCode);
        $this->assertStringContainsString('Service unavailable', $response->message);
    }

    public function test_agent_otp_verification_with_invalid_json(): void
    {
    ...
    public function test_agent_otp_verification_with_network_error(): void
    {
        $mockClient = Mockery::mock(Client::class);
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('Failed agent OTP verification', $response->message);
    }

    private function requestDto(): AgentOtpVerificationRequestDTO
    {
    ...
    private function makeService(array $logging, bool $audit, Client $client): OnboardingService
    {
        ...
        $property = (new \ReflectionClass($service))->getProperty('agentOtpVerificationClient');
