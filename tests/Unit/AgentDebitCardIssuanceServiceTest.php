...
class AgentDebitCardIssuanceServiceTest extends TestCase
{
    ...
    public function test_successful_agent_debit_card_issuance(): void
    {
        ...
        $response = $service->agentDebitCardIssuance($this->requestDto());

        $this->assertInstanceOf(AgentDebitCardIssuanceResponseDTO::class, $response);
        $this->assertTrue($response->success);
        $this->assertEquals('99', $response->id);
    }

    public function test_agent_debit_card_issuance_with_portal_error(): void
    {
        $service = $this->makeService(
            logging: ['logInfo', 'logRequest', 'logResponse'],
            audit: true,
        ...
        $this->assertEquals('9005', $response->errorCode);
        $this->assertStringContainsString('already in process', $response->message);
    }

    public function test_agent_debit_card_issuance_with_invalid_json(): void
    {
    ...
    public function test_agent_debit_card_issuance_with_network_error(): void
    {
        $mockClient = Mockery::mock(Client::class);
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('Failed agent debit card issuance', $response->message);
    }

    private function requestDto(): AgentDebitCardIssuanceRequestDTO
    {
    ...
    private function makeService(array $logging, bool $audit, Client $client): OnboardingService
    {
        ...
        $property = (new \ReflectionClass($service))->getProperty('agentDebitCardIssuanceClient');
