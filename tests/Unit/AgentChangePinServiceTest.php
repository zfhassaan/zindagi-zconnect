...
class AgentChangePinServiceTest extends TestCase
{
    ...
    public function test_successful_agent_change_pin(): void
    {
        ...
        $response = $service->agentChangePin($this->requestDto());

        $this->assertInstanceOf(AgentChangePinResponseDTO::class, $response);
        $this->assertTrue($response->success);
        $this->assertEquals('99', $response->id);
    }

    public function test_agent_change_pin_with_portal_error(): void
    {
        $service = $this->makeService(
            logging: ['logInfo', 'logRequest', 'logResponse'],
            audit: true,
    ...
    public function test_agent_change_pin_with_invalid_json(): void
    {
    ...
    public function test_agent_change_pin_with_network_error(): void
    {
        $mockClient = Mockery::mock(Client::class);
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('Failed agent change pin', $response->message);
    }

    private function requestDto(): AgentChangePinRequestDTO
    {
    ...
    private function makeService(array $logging, bool $audit, Client $client): OnboardingService
    {
        ...
        $property = (new \ReflectionClass($service))->getProperty('agentChangePinClient');
