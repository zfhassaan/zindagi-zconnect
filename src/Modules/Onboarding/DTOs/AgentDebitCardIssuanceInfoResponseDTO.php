...
class AgentDebitCardIssuanceInfoResponseDTO
{
    /**
    ...
    {
        $wrapperKey = self::resolveWrapperKey($response);
        $hasWrapper = $wrapperKey !== null;
        ...
        $data = $hasWrapper ? $response[$wrapperKey] : $response;
        ...
        return new self(
            ...
            id: $id,
            errors: $errors,
            message: $firstError['message'] ?? ($success ? 'Debit card issuance info retrieved successfully' : null),
            errorCode: $firstError['code'] ?? ($success ? null : $id),
            originalResponse: $response
        );
    }

    protected static function resolveWrapperKey(array $response): ?string
    {
        foreach (['agentDebitCardIssuanceInfoRes', 'agentDebitCardIssuanceInfoReq'] as $key) {
